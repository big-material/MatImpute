...
def make_col_null_with_ratio(df, col, ratio):
    """
    Make a column null with a given ratio
    :param df: dataframe
    :param col: column name
    :param ratio: ratio of null values
    :return: dataframe
    """
    df = df.copy()
...
def filled_result(df_full, df_filled, df_with_null, miss_col):
    """
...
def check_column_is_long_tail_distribution(df, col):
    """
    Check if a column is long tail distribution
    :param df: the dataframe
    :param col: the column name
    """
    return skew(df[col].dropna()) > 1


def check_column_is_thin(df, col):
    """
    Check if a column is thin
    """
    return kurtosis(df[col].dropna()) > 3


def check_column_is_distinct(df, col):
    """
    Check if a column is distinct
    """
    return len(df[col].unique()) / df.shape[0] < 0.05 and len(df[col].unique()) <= 3


def get_miss_ratio(df, miss_col):
    """
    Get the missing ratio of a column
    :param df: the dataframe
    :param miss_col: the missing column name
    :return: the missing ratio
    """
    return df[miss_col].isnull().sum() / df.shape[0]
